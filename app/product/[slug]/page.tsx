...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    <section className={`product-detail-hero product-detail-hero--${product.collection}`}><div className="shell product-detail-grid"><div className="gallery"><div className="gallery-main"><div className="large-paper-object"><span>LA VATRA</span><h1>{product.title}</h1><p>{product.subtitle}</p><small>{product.type}</small></div></div><div className="gallery-thumbs"><div>Cover</div><div>Inside</div><div>Detail</div></div></div><div className="product-info"><span className="product-badge">{product.badge}</span><h1>{product.title}</h1><p className="product-subtitle">{product.subtitle}</p><p>{product.description}</p><div className="product-spec"><span>Format</span><strong>{product.formats}</strong></div><button className="button button--dark product-buy">Add to your hearth <small>Shop connection coming next</small></button><p className="microcopy">Digital product · No physical item is shipped unless explicitly stated.</p></div></div></section>
